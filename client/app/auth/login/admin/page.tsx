...
import { Shield, ArrowLeft, Eye, EyeOff, Loader2, Settings, Users, Database } from 'lucide-react';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (mockUser && mockUser.role === 'ADMIN') {
        ...
        setError('Invalid credentials. Use: edusphereadmin@gmail.com / admin123');
      }
    } finally {
      setIsLoading(false);
    }
    ...
    <div className="min-h-screen bg-gradient-to-br from-purple-50 via-violet-50 to-indigo-50 flex items-center justify-center p-4">
      <div className="w-full max-w-6xl grid lg:grid-cols-2 gap-8 items-center">
          ...
          <div className="space-y-6">
            ...
            >
              {[
                ...
                { icon: Database, text: 'Data Administration' }
              ].map((item, index) => (
                <motion.div
                  key={index}
        ...
        <motion.div
          ...
          transition={{ duration: 0.6 }}
        >
          <Card className="p-8 shadow-2xl border-2 border-purple-100 bg-white/80 backdrop-blur-sm">
            <div className="mb-8">
              <button
                onClick={() => router.push('/auth/role-selection')}
                className="flex items-center gap-2 text-slate-600 hover:text-purple-600 transition-colors mb-6"
              >
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <Button
                type="submit"
                disabled={isLoading}
                className="w-full h-12 bg-gradient-to-r from-purple-500 to-purple-600 hover:from-purple-600 hover:to-purple-700 text-white font-bold text-lg shadow-lg hover:shadow-xl transition-all duration-300"
              >
                {isLoading ? (
                  <>
            ...
            <div className="mt-8 pt-6 border-t border-slate-200">
              <div className="bg-purple-50 border border-purple-200 rounded-xl p-4 mb-4">
                <p className="text-xs font-semibold text-purple-700 mb-2">🔐 Test Credentials:</p>
                <p className="text-xs text-purple-600 font-mono">
                  <span className="font-bold">Email:</span> edusphereadmin@gmail.com<br />
                  <span className="font-bold">Password:</span> admin123
                </p>
              </div>
              <div className="flex items-center justify-center gap-2 text-sm text-slate-600">
                <Shield className="h-4 w-4 text-purple-600" />
                <span className="font-medium">Secure Administrator Access</span>
              </div>
            </div>
          </Card>
        </motion.div>
